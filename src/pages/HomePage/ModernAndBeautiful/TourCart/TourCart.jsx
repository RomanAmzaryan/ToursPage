...
export default function TourCart({ searchData }) {
  return (
    <>
      {searchData.map((tour) => {
        return (
          <div className="cart" key={tour.id}>
            ...
            <div className="padding">
              ...
              <div className="daysCount">
                ...
                {tour.daysQty} days |<i className="bx bx-map-alt map" style={{ color: "#aaaaaa" }}></i>
                G87P,
                {tour.country}
              </div>
              <p className="cartDescription">{tour.tourDetails}</p>
              <CartsButton
                toLink={`/` + ALLCONTINENTS_PAGE_PATH + `/` + CHOOSEN_TOUR_ID + `/${tour.id}`}
                text={"See Detalis"}
